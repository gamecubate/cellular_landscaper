...
// lib/game/cellular-automaton.js
ig.baked=true;ig.module('game.cellular-automaton').requires('impact.game').defines(function(){CellState={DEAD:0,ALIVE:1};CellularAutomaton=ig.Class.extend({cols:0,rows:0,inputData:null,data:null,init:function(cols,rows,data)
...
this.data[row][col]=CellState.DEAD;}}},seed:function(coverage,padding)
...
this.data[row][col]=CellState.ALIVE;}}},step:function()
...
{found+=1;}}
return found;}});Conway=CellularAutomaton.extend({next:function()
...
{nextStates[row][col]=CellState.ALIVE;}}}
this.data=nextStates;return this.data;}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.background-map','game.cellular-automaton','plugins.symbols.symbols','plugins.outlinedfont','plugins.impact-splash-loader').defines(function(){MapMaker=ig.Game.extend({clearColor:null,gameState:null,COLS:40,ROWS:60,PADDING:0,ca:null,ground:null,simulationTimer:null,stepDuration:0.1,font:new OutlinedFont('media/outlinedfont.png',1),init:function()
{new ig.Symbols("PLAYING PAUSED");this.initAutomata();this.initMaps();this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY._1,'1');ig.input.bind(ig.KEY._2,'2');ig.input.bind(ig.KEY._3,'3');ig.input.bind(ig.KEY._4,'4');ig.input.bind(ig.KEY._5,'5');ig.input.bind(ig.KEY.SPACE,'pause');ig.input.bind(ig.KEY.MOUSE1,'mouse1');this.gameState=ig.Entity.PLAYING;},initAutomata:function()
{this.ca=new Conway(this.COLS,this.ROWS);this.ca.seed(0.15);},initMaps:function()
{var data=this.createData(this.COLS,this.ROWS,1);var bg=new ig.BackgroundMap(8,data,new ig.Image('media/bg-full.png'));bg.preRender=true;this.backgroundMaps.push(bg);data=this.createData(this.COLS,this.ROWS);this.ground=new ig.BackgroundMap(8,data,new ig.Image('media/landscape-full-beige.png'));this.backgroundMaps.push(this.ground);},createData:function(cols,rows,fillValue)
...
this.handleKeys();this.handleMouse();this.parent();},updateSimulation:function(automaton,map)
{if(this.simulationTimer.delta()>0&&this.stepDuration>0)
{var states=automaton.data;var newStates=automaton.next();for(var row=this.PADDING;row<this.ROWS-this.PADDING;row++)
{for(var col=this.PADDING;col<this.COLS-this.PADDING;col++)
{var state=states[row][col];var newState=newStates[row][col];var mapTileIndex=map.data[row][col];var maxIndex=map.tiles.width/map.tilesize;if(state==CellState.DEAD&&newState==CellState.ALIVE)
{if(mapTileIndex<maxIndex)
{map.data[row][col]=mapTileIndex+1;}}}}
this.simulationTimer.set(this.stepDuration);}},handleKeys:function()
...
this.stepDuration=0.05;else if(ig.input.pressed('pause'))
this.gameState=this.gameState==ig.Entity.PAUSED?ig.Entity.PLAYING:ig.Entity.PAUSED;},handleMouse:function()
{if(ig.input.released("mouse1"))
{this.ground.data=this.createData(this.COLS,this.ROWS);this.ca.reset();this.ca.seed(0.15,this.PADDING);this.gameState=ig.Entity.PLAYING;}},draw:function()
{ig.system.context.clearRect(0,0,ig.system.realWidth,ig.system.realHeight);this.parent();this.font.draw('MOUSE: RESET     1-5: SPEED     SPACE: PAUSE/PLAY',ig.system.width/2,5,ig.Font.ALIGN.CENTER);},});ig.main('#canvas',MapMaker,30,320,480,1,ig.ImpactSplashLoader);});