...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.background-map','game.cellular-automaton','plugins.symbols.symbols','plugins.impact-splash-loader').defines(function(){MapMaker=ig.Game.extend({clearColor:null,gameState:null,COLS:40,ROWS:60,PADDING:0,ca:null,ground:null,simulationTimer:null,stepDuration:0.1,init:function()
{new ig.Symbols("PLAYING PAUSED");this.initAutomatons();this.initMaps();this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY._1,'1');ig.input.bind(ig.KEY._2,'2');ig.input.bind(ig.KEY._3,'3');ig.input.bind(ig.KEY._4,'4');ig.input.bind(ig.KEY._5,'5');ig.input.bind(ig.KEY.SPACE,'pause');ig.input.bind(ig.KEY.MOUSE1,'mouse1');this.gameState=ig.Entity.PLAYING;},initAutomatons:function()
...
{ig.system.context.clearRect(0,0,ig.system.realWidth,ig.system.realHeight);this.parent();},});ig.main('#canvas',MapMaker,30,320,480,1,ig.ImpactSplashLoader);});